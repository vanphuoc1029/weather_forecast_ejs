...
const app = express();
const port = 3000;
const weatherAPIKey = "f429e3206e794b49be290719241503";
const weatherBaseURL = "http://api.weatherapi.com/v1";
const HCMcoordinate = "10.762622,106.660172"
const sunnyReminders = ["Don't forget to wear sunscreen", "Stay hydrated", "Wear sunglasses", "Wear a hat"];
const rainyReminders = ["Bring an umbrella", "Wear waterproof shoes", "Stay dry"];
...
app.get("/", async (req, res) => {
    try {
        const weather = await axios.get(`${weatherBaseURL}/current.json?key=${weatherAPIKey}&q=${HCMcoordinate}`);
        ...
        { condition: condition, temp: temp, humidity: humidity, uv: uv, reminder: reminder, icon : icon});
    }
    catch (error) {
        console.log(error);
    }
